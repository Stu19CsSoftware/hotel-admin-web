    ...
    <div>
        <el-card shadow="always">
            <div slot="header" class="horiz-container align-center">
                <span class="header-text">客房管理</span>
            ...
            >
                ...
                <el-table-column prop="title" label="房号" width="100"></el-table-column>
                <el-table-column prop="Roomtype" label="房型" width="150"></el-table-column>
                <el-table-column prop="content" label="客房设施" ></el-table-column>
                <el-table-column prop="Roomprice" label="房间价格" width="90"></el-table-column>
                <el-table-column prop="RatedPerson" label="额定人数" width="80"></el-table-column>
        ...
        >
            ...
            <el-card shadow="always">
                <el-form label-position="top">
                    ...
                    <el-form-item label="客房设施(支持 Markdown 语法)">
                        <el-input
                            type="textarea"
                    ...
                    <el-form-item label="额定人数">
                      <el-input
                          type="number"
                          v-model="editedItem.RatedPerson"
                          controls-position="right"
                          :max="10"
                          label="RatedPerson"
                      ></el-input>
...
<script>
import RoomService from '../../../api/service/room_service';
...
export default {
    ...
    methods: {
        async getRoom() {
            ...
            // 查询当前页的数据，
            let response = await RoomService.fetchAll(this.pageSize, skip, ["createdBy", "lastEditor"], ['-level', '-updatedAt']);
        ...
        async saveItem() {
            ...
            if (this.editedIndex === -1) {
                ...
                */
                ...
                // 添加一个创建者的属性
                this.editedItem.createdBy = await RoomService.currentUser();
                // 添加一个最后修改者的属性
                this.editedItem.lastEditor = await RoomService.currentUser();
                // 添加一个最后回复时间的属性
               ...
               // this.editedItem.RatedPerson = await TopicService.currentUser();
                ...
                if (this.editedItem.title) {
                    // 向服务器添加一条记录
                    let response = await RoomService.create(this.editedItem);
                    if (response) {
                        // 新增的记录添加到话题列表的前面
                        ...
                        // 使用 message 通知 
                        this.$message({
                            message: `已经成功添加一个客房: ${this.editedItem.title}`,     // 消息内容
                            type: 'success'
                        });
                ...
                let response = await RoomService.update(item.id, {
                    title: item.title,           // 修改后的标题                
                    content: item.content,       // 修改后的话题内容
                    level: Number(item.level),   // 修改后的level，
                    RatedPerson: Number(item.RatedPerson),  // 要转为数字类型
                    lastEditor: await RoomService.currentUser(),  // 最后修改者为当前用户
        ...
        // 删除单条记录
        deleteItem(item) {
            this.$confirm(`确定要删除房号为："${item.title}" 的客房吗？`, '删除客房', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(async () => {  // .then()  里面的匿名函数前要加  async                 
                // 在服务器端执行删除操作
                let response = await RoomService.delete(item.id);
                if (response) {
                    this.getRoom();  // 重新载入数据 
                    this.$message({
                        type: 'success',
        ...
        async deleteSelected() {
            this.$confirm(`确定要删除已选中的 ${this.selected.length} 个客房吗？`, '批量删除客房', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(async () => {     // .then()  里面的匿名函数前要加  async 
                // 在服务器端执行删除操作
                let response = await RoomService.deleteBatch(this.selected);
                if (response) {
                    this.getRoom();  // 重新载入数据 
                    this.$message({
                        type: 'success',
                        message: `成功删除选中的 ${this.selected.length} 个客房!`
                    });
        ...
        toJson(item) {
            return {
                ...
                RatedPerson: item.get('RatedPerson'),
                Roomtype: item.get('Roomtype'),
                Roomprice:item.get('Roomprice'),
